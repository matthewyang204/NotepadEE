...
build:
	...
	export ARCH && \
	nuitka --output-dir=dist --target=x86_64-macos10.9 --include-module=tkinter --enable-plugin=tk-inter --tcl-library-dir=$(shell brew --prefix tcl-tk)/lib --tk-library-dir=$(shell brew --prefix tcl-tk)/lib --macos-create-app-bundle --macos-target-arch="$$ARCH" --macos-app-icon=Notepad.png --macos-app-name="Notepad==" --macos-signed-app-name="com.matthewyang.NotepadEE" --macos-app-version=5.0.9 src/Notepad==.py
